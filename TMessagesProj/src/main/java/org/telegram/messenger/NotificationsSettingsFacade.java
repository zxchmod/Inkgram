...
//                    settings = new TLRPC.TL_notificationSoundDefault();
    ...
    // --- Inkgram Theme Color Methods ---

    private int getSavedLightColor() {

        SharedPreferences prefs = getSharedPreferences("InkgramTheme", MODE_PRIVATE);

        return prefs.getInt("lightColor", Color.parseColor("#FFFFFF"));

    }



    private int getSavedDarkColor() {

        SharedPreferences prefs = getSharedPreferences("InkgramTheme", MODE_PRIVATE);

        return prefs.getInt("darkColor", Color.parseColor("#000000"));

    }



    private void saveLightColor(int color) {

        SharedPreferences prefs = getSharedPreferences("InkgramTheme", MODE_PRIVATE);

        prefs.edit().putInt("lightColor", color).apply();

        applyLightThemeColor(color);

    }



    private void saveDarkColor(int color) {

        SharedPreferences prefs = getSharedPreferences("InkgramTheme", MODE_PRIVATE);

        prefs.edit().putInt("darkColor", color).apply();

        applyDarkThemeColor(color);

    }



    private void applyLightThemeColor(int color) {

        findViewById(R.id.rootLayout).setBackgroundColor(color);

    }



    private void applyDarkThemeColor(int color) {
